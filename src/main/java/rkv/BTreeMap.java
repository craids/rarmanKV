...
class BTreeMap<K, V> extends AbstractMap<K, V> implements ConcurrentNavigableMap<K, V> {
	...
	protected BTreeMap(BTree<K, V> tree, boolean readonly, K fromKey, boolean fromInclusive, K toKey,
			boolean toInclusive) {
		this.tree = tree;
	...
	private final Set<java.util.Map.Entry<K, V>> _entrySet = new AbstractSet<Entry<K, V>>() {
		...
		public Iterator<java.util.Map.Entry<K, V>> iterator() {
			try {
				final BTree.BTreeTupleBrowser<K, V> br = fromKey == null ? tree.browse()
						: tree.browse(fromKey, fromInclusive);
