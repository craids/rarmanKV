...
class SegmentNonMemoryPool<V> extends Segment<V> {
	...
	V getEntry(KeyBuffer key) {
		boolean wasFirst = lock();
		try {
			for (long hashEntryAdr = table.getFirst(
					key.hash()); hashEntryAdr != 0L; hashEntryAdr = NonMemoryPoolHashEntries.getNext(hashEntryAdr)) {

				if (key.sameKey(hashEntryAdr)) {
					hitCount++;
					return valueSerializer.deserialize(
							Uns.readOnlyBuffer(hashEntryAdr, fixedValueLength, NonMemoryPoolHashEntries.ENTRY_OFF_DATA
									+ NonMemoryPoolHashEntries.getKeyLen(hashEntryAdr)));
				}
			}
	...
	private boolean putEntry(long newHashEntryAdr, long hash, long keyLen, boolean putIfAbsent, long oldValueAddr) {
		...
		try {
			...
			for (hashEntryAdr = table.getFirst(
					hash); hashEntryAdr != 0L; prevEntryAdr = hashEntryAdr, hashEntryAdr = NonMemoryPoolHashEntries
				...
				if (oldValueAddr != 0L) {
					...
					if (!Uns.memoryCompare(hashEntryAdr, NonMemoryPoolHashEntries.ENTRY_OFF_DATA + keyLen, oldValueAddr,
							0L, fixedValueLength)) {
						return false;
					}
				}
	...
	private static boolean notSameKey(long newHashEntryAdr, long newHash, long newKeyLen, long hashEntryAdr) {
		...
		return serKeyLen != newKeyLen || !Uns.memoryCompare(hashEntryAdr, NonMemoryPoolHashEntries.ENTRY_OFF_DATA,
				newHashEntryAdr, NonMemoryPoolHashEntries.ENTRY_OFF_DATA, serKeyLen);
	}
	...
	private void serializeForPut(byte[] key, V value, long hashEntryAdr) {
		try {
			...
			if (value != null) {
				valueSerializer.serialize(value, Uns.buffer(hashEntryAdr, fixedValueLength,
						NonMemoryPoolHashEntries.ENTRY_OFF_DATA + key.length));
			}
		} catch (Throwable e) {
	...
	boolean removeEntry(KeyBuffer key) {
		...
		try {
			long prevEntryAdr = 0L;
			for (long hashEntryAdr = table.getFirst(key
					.hash()); hashEntryAdr != 0L; prevEntryAdr = hashEntryAdr, hashEntryAdr = NonMemoryPoolHashEntries
							.getNext(hashEntryAdr)) {
				if (!key.sameKey(hashEntryAdr)) {
					continue;
	...
	private void rehash() {
		...
		for (int part = 0; part < tableSize; part++) {
			for (long hashEntryAdr = tab.getFirst(part); hashEntryAdr != 0L; hashEntryAdr = next) {
				...
				long hash = hasher.hash(hashEntryAdr, NonMemoryPoolHashEntries.ENTRY_OFF_DATA,
						NonMemoryPoolHashEntries.getKeyLen(hashEntryAdr));
	...
	void getEntryAddresses(int mapSegmentIndex, int nSegments, LongArrayList hashEntryAdrs) {
		...
		try {
			for (; nSegments-- > 0 && mapSegmentIndex < table.size(); mapSegmentIndex++) {
				for (long hashEntryAdr = table
						.getFirst(mapSegmentIndex); hashEntryAdr != 0L; hashEntryAdr = NonMemoryPoolHashEntries
								.getNext(hashEntryAdr)) {
	...
	static final class Table {
		...
		void clear() {
			...
			// (uninitialized memory will cause problems - endless loops, JVM crashes,
			// damaged data, etc)
		...
		private void removeLinkInternal(long hash, long hashEntryAdr, long prevEntryAdr, long next) {
			...
			if (head == hashEntryAdr) {
				...
				if (prevEntryAdr == -1L) {
					for (long adr = head; adr != 0L; prevEntryAdr = adr, adr = NonMemoryPoolHashEntries.getNext(adr)) {
						if (adr == hashEntryAdr) {
							break;
						}
					}
