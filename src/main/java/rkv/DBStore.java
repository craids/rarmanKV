...
class DBStore extends DBAbstract {
	...
	public DBStore(String filename, boolean readonly, boolean transactionDisabled, boolean lockingDisabled)
			throws IOException {
		this(filename, readonly, transactionDisabled, null, null, false, false, false);
	}

	public DBStore(String filename, boolean readonly, boolean transactionDisabled, Cipher cipherIn, Cipher cipherOut,
			boolean useRandomAccessFile, boolean deleteFilesAfterClose, boolean lockingDisabled) {
		_filename = filename;
	...
	private void reopen() {
		try {
			...
			if (versionNumber > STORE_FORMAT_VERSION)
				throw new IOException("Unsupported version of store. Please update JDBM. Minimal supported ver:"
	...
	boolean needsAutoCommit() {
		return transactionsDisabled && !commitInProgress && (_file.getDirtyPageCount() >= AUTOCOMMIT_AFTER_N_PAGES);
	}
	...
	private <A> long insert2(A obj, Serializer<A> serializer, DataInputOutput buf) throws IOException {
		...
		if (obj == PREALOCATE_OBJ) {
			...
			if (buf.getPos() > RecordHeader.MAX_RECORD_SIZE) {
				throw new IllegalArgumentException("Too big record. JDBM only supports record size up to: "
						+ RecordHeader.MAX_RECORD_SIZE + " bytes. Record size was: " + buf.getPos());
			}
	...
	private <A> void update2(long logRecid, final A obj, final Serializer<A> serializer, final DataInputOutput buf)
		...
		long newRecid = physRecid != PREALOCATE_PHYS_RECID ? _physMgr.update(physRecid, buf.getBuf(), 0, buf.getPos()) :
	...
	public synchronized <A> A fetch(final long recid, final Serializer<A> serializer) throws IOException {
	...
	private <A> A fetch2(long recid, final Serializer<A> serializer, final DataInputOutput buf) throws IOException {
	...
	public void copyToZip(String zipFile) {
		try {
			...
			for (long pageid = _pageman.getFirst(Magic.USED_PAGE); pageid != 0; pageid = _pageman.getNext(pageid)) {
				PageIo page = _file.get(pageid);
				String file = zip2 + pageid;
				z.putNextEntry(new ZipEntry(file));
				z.write(Utils.encrypt(cipherIn, page.getData()));
	...
	private long statisticsCountPages(short pageType) throws IOException {
		...
		for (long pageid = _pageman.getFirst(pageType); pageid != 0; pageid = _pageman.getNext(pageid)) {
			pageCounter++;
		}
		...
		try {
			...
			// count pages
			{
				...
				b.append("  " + pages + " record translation pages with size "
						+ Utils.formatSpaceUsage(pages * Storage.PAGE_SIZE) + "\n");
				...
				b.append("  Total number of pages is " + total + " with size "
						+ Utils.formatSpaceUsage(total * Storage.PAGE_SIZE) + "\n");
			...
			}
			{
				...
				b.append("  Average data size in record is "
						+ Utils.formatSpaceUsage(Math.round(1D * totalRecordSize / recordCount)) + "\n");
				...
				b.append("  Maximal space wasted in single record fragmentation is "
						+ Utils.formatSpaceUsage(maximalAvailSizeDiff) + "\n");
			}
	...
	public synchronized void defrag(boolean sortCollections) {
		...
		try {
			...
			// recreate logical file with original page layout
			{
				...
				long pageCounter = 0;
				for (long pageid = db2._pageman
						.allocate(Magic.TRANSLATION_PAGE); pageid >= minpageid; pageid = db2._pageman
			...
			for (long pageid = _pageman.getFirst(Magic.TRANSLATION_PAGE); pageid != 0; pageid = _pageman
				...
				for (int i = 0; i < _logicMgr.ELEMS_PER_PAGE; i += 1) {
					...
					// force insert into other file, without decompressing logical id to external
					// form
	...
	static long decompressRecid(final long recid) {
		...
		final short offset = (short) ((recid & COMPRESS_RECID_OFFSET_MASK) * Magic.PhysicalRowId_SIZE
				+ Magic.PAGE_HEADER_SIZE);
