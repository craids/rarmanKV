...
final class OffHeapHashTableImpl<V> implements OffHeapHashTable<V> {
	...
	private boolean putInternal(byte[] key, V value, boolean ifAbsent, V old) {
		...
		if (valueSize != fixedValueLength) {
			throw new IllegalArgumentException(
					"value size " + valueSize + " greater than fixed value size " + fixedValueLength);
		}

		if (old != null && valueSize(old) != fixedValueLength) {
			throw new IllegalArgumentException(
					"old value size " + valueSize(old) + " greater than fixed value size " + fixedValueLength);
		}

		if (key.length > Byte.MAX_VALUE) {
			throw new IllegalArgumentException(
					"key size of " + key.length + " exceeds max permitted size of " + Byte.MAX_VALUE);
		}
	...
	public OffHeapHashTableStats stats() {
		long hitCount = 0, missCount = 0, size = 0, freeCapacity = 0, rehashes = 0, putAddCount = 0,
				putReplaceCount = 0, removeCount = 0;
		for (Segment map : segments) {
		...
		return new OffHeapHashTableStats(hitCount, missCount, size, rehashes, putAddCount, putReplaceCount,
				putFailCount, removeCount, perSegmentStats());
	}
