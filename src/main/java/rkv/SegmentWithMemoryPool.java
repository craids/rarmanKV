...
class SegmentWithMemoryPool<V> extends Segment<V> {
	...
	public V getEntry(KeyBuffer key) {
		...
		try {
			for (MemoryPoolAddress address = table.getFirst(key.hash()); address.chunkIndex >= 0; address = getNext(
					address)) {
	...
	public boolean removeEntry(KeyBuffer key) {
		...
		try {
			...
			for (MemoryPoolAddress address = table
					.getFirst(key.hash()); address.chunkIndex >= 0; previous = address, address = getNext(address)) {
	...
	private MemoryPoolAddress getNext(MemoryPoolAddress address) {
		if (address.chunkIndex < 0 || address.chunkIndex >= chunks.size()) {
			throw new IllegalArgumentException(
					"Invalid chunk index " + address.chunkIndex + ". Chunk size " + chunks.size());
		}
	...
	private MemoryPoolAddress writeToFreeSlot(byte[] key, byte[] value, MemoryPoolAddress nextAddress) {
		...
		if (currentChunkIndex == -1 || chunks.get(currentChunkIndex).remaining() < fixedSlotSize) {
			...
			// There is no chunk allocated for this segment or the current chunk being
			// written to has no space left.
			// allocate an new one.
	...
	static final class Table {
		...
		void updateBucketHistogram(EstimatedHistogram h, final List<MemoryPoolChunk> chunks) {
			for (int i = 0; i < size(); i++) {
				int len = 0;
				for (MemoryPoolAddress adr = getFirst(i); adr.chunkIndex >= 0; adr = chunks.get(adr.chunkIndex)
						.getNextAddress(adr.chunkOffset)) {
					len++;
				}
				h.add(len + 1);
			}
