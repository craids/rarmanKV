...
final class BTreeNode<K, V> implements Serializer<BTreeNode<K, V>> {
	...
	BTree.BTreeTupleBrowser<K, V> find(int height, final K key, final boolean inclusive) throws IOException {
...
//          // check if we have an exact match
	...
	InsertResult<K, V> insert(int height, K key, final V value, final boolean replace) throws IOException {
		...
		if (height == 0) {
			...
			if (DEBUG) {
				System.out.println(
						"BTreeNode.insert() Insert on leaf node key=" + key + " value=" + value + " index=" + index);
			}
		...
		if (index < half) {
			...
			if (DEBUG) {
				System.out.println(
						"BTreeNode.insert() move lower-half of entries to overflow BTreeNode, including new entry.");
			}
			if (height == 0) {
			...
			if (DEBUG) {
				System.out.println(
						"BTreeNode.insert() move lower-half of entries to overflow BTreeNode. New entry stays");
			}
			if (height == 0) {
	...
	private static <K, V> void copyEntries(BTreeNode<K, V> source, int indexSource, BTreeNode<K, V> dest, int indexDest,
			int count) {
	...
	private static <K, V> void copyChildren(BTreeNode<K, V> source, int indexSource, BTreeNode<K, V> dest,
			int indexDest, int count) {
		System.arraycopy(source._keys, indexSource, dest._keys, indexDest, count);
	...
	public void serialize(DataOutput oos, BTreeNode<K, V> obj) throws IOException {
		...
		// note: It is assumed that BTreeNode instance doing the serialization is the
		// parent
...
//				ret[i] = Serialization.readObject(ois);
	...
	private void writeKeys(DataOutput oos, K[] keys, final int firstUse) throws IOException {
		...
		if ((_btree._comparator == Utils.COMPARABLE_COMPARATOR || _btree._comparator == null)
				&& (_btree.keySerializer == null
						|| _btree.keySerializer == _btree.getRecordManager().defaultSerializer())) {
			...
			for (int i = firstUse; i < BTree.DEFAULT_SIZE; i++) {
				...
				// special case to exclude Long.MIN_VALUE from conversion, causes problems to
				// LongPacker
...
//					Serialization.writeObject(oos, keys[i]);
	...
	static final class Browser<K, V> implements BTree.BTreeTupleBrowser<K, V> {
		...
		public void remove(K key) throws IOException {
			...
			// browser is positioned just before value which was currently deleted, so find
