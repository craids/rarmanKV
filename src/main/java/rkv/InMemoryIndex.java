...
class InMemoryIndex {
	...
	InMemoryIndex(int numberOfKeys, boolean useMemoryPool, int fixedKeySize, int memoryPoolChunkSize) {
		...
		OffHeapHashTableBuilder<InMemoryIndexMetaData> builder = OffHeapHashTableBuilder
				.<InMemoryIndexMetaData>newBuilder().valueSerializer(new InMemoryIndexMetaDataSerializer())
				.segmentCount(noOfSegments).hashTableSize(maxSizeOfEachSegment)
				.fixedValueSize(InMemoryIndexMetaData.SERIALIZED_SIZE).loadFactor(1);
