...
abstract class DBCache extends DBStore {
	...
	static final class DirtyCacheEntry {
		long _recid; // TODO recid is already part of _hashDirties, so this field could be removed to
						// save memory
		Object _obj;
	...
	public DBCache(String filename, boolean readonly, boolean transactionDisabled, Cipher cipherIn, Cipher cipherOut,
			boolean useRandomAccessFile, boolean deleteFilesAfterClose, boolean lockingDisabled) {

		super(filename, readonly, transactionDisabled, cipherIn, cipherOut, useRandomAccessFile, deleteFilesAfterClose,
				lockingDisabled);
	...
	boolean needsAutoCommit() {
		return super.needsAutoCommit() || (transactionsDisabled && !commitInProgress
				&& _hashDirties.size() > NUM_OF_DIRTY_RECORDS_BEFORE_AUTOCOMIT);
	}
		...
		try {
			synchronized (_hashDirties) {
				...
				while (!_hashDirties.isEmpty()) {
					...
					// update may have triggered more records to be added into dirties, so repeat
