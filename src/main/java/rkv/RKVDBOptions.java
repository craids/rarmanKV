...
public class RKVDBOptions implements Cloneable {
	...
	public RKVDBOptions clone() {
		try {
			return (RKVDBOptions) super.clone();
	...
	public String toString() {
		return MoreObjects.toStringHelper("").add("compactionThresholdPerFile", compactionThresholdPerFile)
				.add("maxFileSize", maxFileSize).add("flushDataSizeBytes", flushDataSizeBytes)
				.add("syncWrite", syncWrite).add("isCompactionDisabled", isCompactionDisabled)
				.add("numberOfRecords", numberOfRecords).add("compactionJobRate", compactionJobRate)
				.add("cleanUpInMemoryIndexOnClose", cleanUpInMemoryIndexOnClose)
				.add("cleanUpTombstonesDuringOpen", cleanUpTombstonesDuringOpen).add("useMemoryPool", useMemoryPool)
				.add("fixedKeySize", fixedKeySize).add("memoryPoolChunkSize", memoryPoolChunkSize).toString();
	}
