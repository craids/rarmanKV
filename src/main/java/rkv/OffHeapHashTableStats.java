...
final class OffHeapHashTableStats {
	...
	public OffHeapHashTableStats(long hitCount, long missCount, long size, long rehashCount, long putAddCount,
			long putReplaceCount, long putFailCount, long removeCount, SegmentStats[] segmentStats) {
	...
	public String toString() {
		return Objects.toStringHelper(this).add("hitCount", hitCount).add("missCount", missCount).add("size", size)
				.add("rehashCount", rehashCount)
				.add("put(add/replace/fail)", Long.toString(putAddCount) + '/' + putReplaceCount + '/' + putFailCount)
				.add("removeCount", removeCount).toString();
	}
